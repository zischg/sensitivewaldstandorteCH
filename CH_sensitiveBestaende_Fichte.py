...
#input data national Baumartenanteil
baumartenanteil=gpd.read_file(projectspace+'/baumartenanteil.gpkg')
...
for kanton in kantons:
    print(kanton)
    #kanton ='AI'
    #read input data Kanton
    bk_gdf = gpd.read_file(projectspace + '/' + kanton + '/stok_gdf_attributed.gpkg', layer='stok_gdf_attributed')
    #bk_gdf = bk_gdf[bk_gdf['kanton'] == kanton]
    len(bk_gdf)
    bk_gdf = bk_gdf[['geometry']]
    bk_gdf['joinid'] = bk_gdf.index
    #raster stats Nadelholzanteil
    bk_gdf=bk_gdf[['geometry']]
    bk_gdf['joinid'] = bk_gdf.index
    # attribute raster values to Bestandeskarte
    print('raster stats Nadelholzanteil')
    bk_gdf["LH"] = 0.0
    ...
    for climatescenario in climatescenarios:
        ...
        rcp_baumartenempfehlungen_gdf_in=joblib.load(projectspace+"/"+kanton+"/"+kanton+"_"+climatescenario+"_baumartenempfehlungen.sav")
        ...
        rcp_bk_gdf_out.to_file(projectspace+"/"+kanton+"/"+kanton+"_"+climatescenario+"_sensitivebestaende_FI-LFI_NH-WSL"+varname+".gpkg")
        print(climatescenario + ' done')
        winsound.Beep(frequency, duration)
    print(kanton + ' sensitive Bestaende done')
    winsound.Beep(frequency, duration)
    print('analysis ...')
    #Auswertungen
    sensi45var1 = gpd.read_file(projectspace + "/"+kanton+"/" + kanton+"_" + "rcp45" + "_sensitivebestaende_FI-LFI_NH-WSL" + "var1" + ".gpkg")
    sensi45var1 = sensi45var1[sensi45var1["maxsens"] >= 0]
    sensi85var1 = gpd.read_file(projectspace + "/"+kanton+"/" + kanton+"_" + "rcp85" + "_sensitivebestaende_FI-LFI_NH-WSL" + "var1" + ".gpkg")
    sensi85var1 = sensi85var1[sensi85var1["maxsens"] >= 0]
    #write output
    outfile = open(projectspace + "/"+kanton+"/" + kanton+"_" + "sensitiveBestaende_FI-LFI_NH-WSL_stat.txt", "w")
