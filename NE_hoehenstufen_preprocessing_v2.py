...
len(ne_gdf)
ne_gdf.columns
ne_gdf=ne_gdf[['code_neuch','hohenstufe', "geometry"]]
...
ne_unique=ne_gdf[['code_neuch']].drop_duplicates()
...
for index, row in ne_unique.iterrows():
    tahs_list=[]
    tahs_listshort = []
    stotyp=row["code_neuch"]
    #print(stotyp)
    tempsel=ne_gdf[(ne_gdf["code_neuch"]==stotyp)]
    tahs_list=tempsel["tahs"].unique().tolist()
    for item in tahs_list:
        if item not in tahs_listshort:
            tahs_listshort.append(item)
    ne_unique.loc[((ne_unique["code_neuch"] == stotyp)), "tahs"] = str(tahs_listshort).replace("[","").replace("]","").replace("'","").replace("]","")

ne_unique.to_excel(codespace+"/NE_nais_einheiten_unique_v2.xlsx")